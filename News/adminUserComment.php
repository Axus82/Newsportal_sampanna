...
<html lang = "en">
	<head>
		...
		<title>Elite News - User Comments</title>
	...
	<body>
		...
		<?php
				...
				// selecting all comment from a specific user...
                $statemnt = $connection->prepare("SELECT * FROM `assignment1`.`comment` WHERE full_name = '$name'");

				// executing the selected comments...
                $statemnt->execute();

				// if number of rows in table is greater than zero...
				if ($statemnt->rowCount() > 0){

					// fetching the executed data...
                while ($comResult = $statemnt->fetch(PDO::FETCH_NUM)){
					?>
					<!-- field to show comments along with username of user who commented... -->
			<div class = "comments">
				<!-- field to show username -->
